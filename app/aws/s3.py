...
def s3_key_generator(project_id: str, document_id: str, file_extension: str, base_folder: str ='projects') -> str:
...
async def upload_file_s3(file: UploadFile, s3_bucket: str, s3_key: str, content_type: str, project_id: UUID, document_id: UUID, s3_client: S3Client) -> None:
    await s3_client.upload_fileobj(file.file, s3_bucket, s3_key, ExtraArgs={"ContentType": content_type, "Metadata": {
                "project_id": str(project_id),
                "document_id": str(document_id)}})
    
async def get_download_url_s3(s3_key: str, s3_client: S3Client, expiration: int = 3600) -> str:
...
async def delete_folder_s3(bucket_name: str, folder_prefix: str, s3_client: S3Client) -> None:
