...
from app.core.exceptions import AppExceptionError, DataIntegrityError
from app.core.settings import settings


def get_file_extension_if_allowed(
    filename: str, allowed_extensions: set = settings.ALLOWED_EXTENSIONS
) -> str:
    _, file_extension = os.path.splitext(filename.lower())
    if file_extension not in allowed_extensions:
        raise DataIntegrityError(
            f"Unsupported file extension '{file_extension}'. Allowed: {', '.join(allowed_extensions)}"
        )
...
def validate_upload_file_size(
    file: UploadFile, max_size: int = settings.MAX_ALLOWED_FILE_SIZE_B
):
    file.file.seek(0, 2)
