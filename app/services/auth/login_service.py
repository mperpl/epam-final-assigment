...
async def login_service(response: Response, login_data: UserLoginDTO, db: AsyncSession, valkey: Valkey) -> User:
    stmt = select(User).where(User.email == login_data.email)
    user = (await db.execute(stmt)).scalar_one_or_none()

    if not user or not await verify_password(login_data.password.get_secret_value(), user.password):
