...
async def invite_user_service(project_id: UUID, owner_id: UUID, invitee_data: UserInviteDTO, db: AsyncSession) -> dict:
    ...
    try:
        new_member = ProjectMember(project_id=project_id, user_id=target_user.id, role=role_to_assign)
        db.add(new_member)
        await db.commit()
...
async def _get_target_user(invitee_data: UserInviteDTO, db: AsyncSession) -> User:
        ...
        if not target_user:
            raise EntryNotFoundError(f"No registered user found matching email: {invitee_data.invitee_email}")
        return target_user