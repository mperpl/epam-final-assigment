...
async def download_document_service(project_id: UUID, document_id: UUID, user_id: UUID, db: AsyncSession, s3_client: S3Client) -> DocumentDownloadResponse:
    document: Document = await get_document(project_id, document_id, db)
    await user_role_in(project_id, user_id, (ProjectRole.OWNER, ProjectRole.EDITOR, ProjectRole.VIEWER), db)
