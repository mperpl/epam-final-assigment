...
async def login(response: Response, login_data: UserLoginDTO, db: DB_SESSION, valkey: VALKEY_CLIENT, _: LOGGED_OUT_GUARD):
    return await login_service(response, login_data, db, valkey)

@router.post("/logout", status_code=status.HTTP_200_OK, response_model=UserLogoutResponse)
async def logout(response: Response, request: Request, valkey: VALKEY_CLIENT, user_id: CURRENT_USER_ID):
    await logout_service(response, request, valkey, user_id)
    return {}

@router.post("/logout-all", status_code=status.HTTP_200_OK, response_model=UserLogoutAllResponse)
async def logout_all(response: Response, valkey: VALKEY_CLIENT, user_id: CURRENT_USER_ID):
    await logout_all_service(response, valkey, user_id)
    return {}

@router.post("/refresh", status_code=status.HTTP_200_OK, response_model=UserRefreshResponse)
async def refresh(response: Response, valkey: VALKEY_CLIENT, refresh_token: str = Cookie()):
