...
@pytest.mark.parametrize(
    "incomplete_payload",
    [
        {"password1": "Pass123!", "password2": "Pass123!"},  # Missing email key entirely
        {"email": "", "password1": "Pass123!", "password2": "Pass123!"},  # Empty email string
        {"email": "valid@test.com", "password1": "", "password2": "Pass123!"},  # Empty password
    ]
)
@pytest.mark.asyncio
async def test_register_empty_or_missing_fields_fail(client: AsyncClient, incomplete_payload: dict):
...
@pytest.mark.asyncio
async def test_register_user_duplicate_email_fails(client: AsyncClient, db_session: AsyncSession, test_user_id: UUID):
