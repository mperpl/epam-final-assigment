...
def enforce_project_quota(bucket_name: str, s3_key: str, project_id: str) -> dict:
    ...
    if total_size > HARD_STORAGE_LIMIT:
        print(
            f"HARD LIMIT BREACH: Project {project_id} total is {total_size} bytes. Limit is {HARD_STORAGE_LIMIT} bytes."
        )
        ...
        print(
            f"WARNING: Project {project_id} has exceeded its soft quota allocation \
                ({total_size} / {SOFT_STORAGE_LIMIT} bytes)."
        )
        return {
            "status": "soft_warning",
            "total_size_bytes": total_size,
